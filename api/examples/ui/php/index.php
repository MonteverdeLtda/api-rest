...
class PHP_CRUD_UI
{
    ...
    public function call($method, $url, $data = false)
    {
        ...
        if ($data) {
            ...
            $headers[] = 'Authorization:Basic user1:pass1';
            curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
			curl_setopt($ch, CURLOPT_USERPWD, "user1:pass1");
        }
        ...
        return json_decode($response, true);
		/*
		$this->call('POST', $url . '/login', json_encode("{
			\"username\":\"user1\",
			\"password\":\"pass1\"
		}"));*/
    }
    ...
    public function runBeta()
    {
        ...
        $html .= '</div>';
        $html .= $this->foot();
        return $this->getDefinition($config, $url);
    }
}
...
$ui = new PHP_CRUD_UI(array(
    'url' => 'https://api.monteverdeltda.com',
));
