...
class PHP_CRUD_UI
{
    ...
    {
		global $dir_global;
        $template = file_get_contents($dir_global.'/templates/home.html');
    ...
    public function executeCommand()
    {
		...
		if(isset($definition['code']) && $definition['code'] == 1011){
			if($method == 'POST'){
				if(isset($post['username']) && isset($post['password'])){
					$this->LogIn($post);
				}
			}else{
				$template = file_get_contents($dir_global.'/templates/login.html');
				return Template::render($template, array('menu' => $menu, 'content' => $content));
			}
		}else{
			$template = file_get_contents($dir_global.'/templates/layout.html');
			return Template::render($template, array('menu' => $menu, 'content' => $content));
		}
    ...
    {
		echo "<h3>Enviando...</h3><p></p>";
		if(isset($post['username']) && isset($post['password'])){
			
			$config = $this->config;
			$url = $this->getUrl($this->config);
			
			$fields = array(
				'username' => urlencode($post['username']),
			...
			$login = $this->call('POST', $url . '/login', $fields_string);
			
			if(isset($login['id']) && $login['id'] > 0){
				echo "Bien!!<hr>";
				$openapi = $this->call('GET', $url . '/openapi', '');
